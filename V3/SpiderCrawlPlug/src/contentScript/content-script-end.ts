chrome.storage.local.get('spiderSwitch', function (result) {
  const spiderSwitch = result.spiderSwitch;
  if (!spiderSwitch) {
...
chrome.storage.local.get(['hookJson', 'HookInput', 'hookUrl', 'hookXhr', 'hookCookie', 'hookType', 'hookDebug'], function (result) {
  const hookJson = result.hookJson;
  console.log("------------------hookJson------------------", hookJson);

  const HookInput = String(result.HookInput);
  const hookUrl = result.hookUrl;
  const hookXhr = result.hookXhr;
  const hookCookie = result.hookCookie;
  const hookType = result.hookType;
  const hookDebug = result.hookDebug;
  // if (hookType !== 'document end') {
  //   return
  // }
  if (!hookJson) {
    // 确保DOM加载后注入
    console.log("------------------正在监Hook JSON------------------")
...
function injectHookCookie(e: any) {
  const script = document.createElement('script');
  script.textContent = `
const iframe = document.createElement('iframe');
iframe.style.display = 'none';
...
function injectScript(targetWindow: any) {
  try {
...
// 内联的注入脚本核心逻辑
function injectionCode(currentWindow: any) {
  'use strict';
